...
_SYSTEM_PROMPT = (
    ...
    "For HTTP actions set type to http and put url, method, body, and headers in extra; use env refs for secrets. "
    "When the request targets a known provider (Jellyfin, Steam), reference its "
    "actions BY NAME in the action 'command' field with type 'provider' "
    "(e.g. command 'jellyfin.search' or 'jellyfin.play'); do NOT invent a URL. "
    "ALWAYS extract any parameter (movie title, show, app name, search term) "
    "into 'entities' as an entity whose name is the parameter, so it binds to "
    "the {query} placeholder. Use {query} in action parameters instead of the "
    "literal value. "
